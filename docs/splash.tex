...
enter option ([0:8], <cr>=0):8
 Enter background colour (by name, e.g. "black") (<cr>=):white
 Enter foreground colour (by name, e.g. "white") (<cr>=):black
 Do you want to plot axes and overlaid text in background colour (default is foreground) ? (<cr>=no):y
\end{verbatim}
...
Please enter your selection now (y axis or option):2
(x axis) (<cr>=1):
 (render) (0=none) ([0:20], <cr>=0):
 (vector plot) (0=none, 7=B, 10=v, 17=J) ([0:17], <cr>=0):
 enter z coordinate of observer (<cr>=1.800):
 enter distance between observer and projection screen ([0.000:], <cr>=0.1800):
 Graphics device/type (? to see list, default /xwin): 
\end{verbatim}
...
Please enter your selection now (y axis or option):2
(x axis) (<cr>=1):
 (render) (0=none) ([0:11], <cr>=0):6
 (vector plot) (0=none, 7=v) ([0:7], <cr>=0):0
...
enter option ([0:9], <cr>=0):1
 Plot gas particles? (<cr>=yes):
 Plot ghost particles? (<cr>=no):
 Plot sink particles? (<cr>=no):y
 >> Plot sink particles on top of rendered plots? (<cr>=no):y
 Plot unknown/dead particles? (<cr>=no):
\end{verbatim}
...
Enter option ([0:8], <cr>=0):8
 Plot scale on co-ordinate plots? (<cr>=no):y
 Enter length of scale in the current x,y,z units (<cr>=1.000):3.0856e15
 Enter text to appear below scale (e.g. '10 AU') (<cr>=1 unit): 0.1 pc
 Enter horizontal position as fraction of viewport ([0.000:1.000], <cr>=0.5000):
 Enter vertical position in character heights above bottom (<cr>=1.000):
\end{verbatim}
...
Please enter your selection now (y axis or option):2
(x axis) (<cr>=1):
 (render) (0=none) ([0:9], <cr>=0):6
 (vector plot) (0=none, 7=B) ([0:7], <cr>=0):
  Graphics device/type (? to see list, default /xw): /xw
\end{verbatim}
...
\begin{verbatim}
enter option ([0:7], <cr>=0):7
 enter column to change units (-2=reset all,-1=quit,0=time) ([-2:9], <cr>=-1):1
 enter x [cm] units (new=old*units) (<cr>=0.1000E+17):668.3893
 enter label amendment (<cr>=[cm]): [AU]
 Apply these units to all coordinates and h? (<cr>=yes):
 Enter unit for 'z' in 3D column integrated plots (<cr>=0.1000E+17):
 Enter label for z integration unit (e.g. [cm]) (<cr>=[cm]):

enter column to change units (-2=reset all,-1=quit,0=time) ([-2:9], <cr>=-1):

save units to file? (<cr>=yes):
...
Use 3D opacity rendering? (<cr>=yes):y
...
Do you want to write a ppm file in addition to PGPLOT output? (<cr>=yes):y
\end{verbatim}
Now we replot the original plot with the new settings as follows:
\begin{verbatim}
Please enter your selection now (y axis or option):2
(x axis) (<cr>=1):
 (render) (0=none) ([0:9], <cr>=6):
 (vector plot) (0=none, 7=v) ([0:7], <cr>=0):
 enter z coordinate of observer (<cr>=53.58):
 enter distance between observer and projection screen ([0.000:], <cr>=5.358):
...
enter approximate surface depth (number of smoothing lengths): ([0.000:], <cr>=2.000):
...
---------------- page setup options -------------------
Enter background colour (by name, e.g. "black") (<cr>=):black
 Enter foreground colour (by name, e.g. "white") (<cr>=):white
 Do you want to plot axes and overlaid text in background colour (default is foreground) ? (<cr>=no):
\end{verbatim}
Now, replotting the same plot again, but this time adjusting the opacity at the prompt:
\begin{verbatim}
enter approximate surface depth (number of smoothing lengths): ([0.000:], <cr>=2.000):200.0
\end{verbatim}
