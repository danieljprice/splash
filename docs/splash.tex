...
\end{verbatim}
 The ``<cr>'' refers to the default value assigned if you just press the return key. The last prompt asks for the PGPLOT device to which output should be directed. A full list of available graphics devices is given in the PGPLOT user guide. Some of the most useful devices are given in table \ref{tab:devices}. In the
...
A more complicated plot is where both the $x-$ and $y-$ axes refer to co-ordinates. For example
\begin{verbatim}
Please enter your selection now (y axis or option):2
(x axis) (<cr>=1): 1
(render) (0=none) ([0:11], <cr>=0):5
(vector plot) (0=none, 8=B) ([0:8], <cr>=0):0
Graphics device/type (? to see list, default /xwin): /xw
\end{verbatim}
...
\begin{verbatim}
enter z position for cross-section slice: ([-8.328:8.327], <cr>=0.000):
\end{verbatim}
...
\subsection{Vector plots}
 A prompt to plot vector arrows on top of rendered plots (or on top of particle plots) appears whenever vectors are present in the data (see \S\ref{sec:dataread}).
...
\begin{verbatim}
 Start at timestep ([1:10], <cr>=1):
 End at timestep ([1:10], <cr>=10):
 Frequency of steps to read ([1:10], <cr>=1):
\end{verbatim}
...
\begin{verbatim}
 Enter number of steps to plot ([1:10], <cr>=0):5
 Enter step  1 ([1:10], <cr>=1):5
 Enter step  2 ([1:10], <cr>=2):2
 Enter step  3 ([1:10], <cr>=3):1
 Enter step  4 ([1:10], <cr>=4):4
 Enter step  5 ([1:10], <cr>=5):3
\end{verbatim}
...
The settings for transforming the data into physical units may be changed via the ``change physical unit settings'' option in the d)ata menu. To apply the physical units to the data select the ``use physical units'' option in the d)ata submenu.
 ...
 The transformation used is $new= old*units$ where ``old'' is the data as read from the dump file and ``new'' is the value actually plotted. The data menu option also prompts for a units label which is appended to the usual label. Brackets and spaces should be explicitly included in the label as required.
  ...
  Once units have been changed, the user is prompted to save the unit settings to a file called `splash.units'. This file is straightforward to edit by hand. To revert to the default unit settings simply delete this file. To revert to code units turn ``use physical units'' off in the d)ata menu.
...
[\times 10^{-4}]$ by setting the units label to $y [ \times 10^{-4}]$.
...
\end{tabular}
\caption{Example of particles coloured interactively using the mouse (left) and selection using a parameter range (right), which is the same as the plot on the left but showing only the ``high density'' particles (after an intermediate plot of density vs x on which I selected a subset of particles and hit 'p')}
...
\subsubsection{ Plotting only a subset of the particles}
 To turn plotting of certain particle \emph{types} on and off, see \S\ref{sec:plotparticlesbytype}. To interactively select a subset of the particles to plot, see \S\ref{sec:colourparts}.

\subsubsection{ Rendering using only a subset of the particles}
See \S\ref{sec:colourparts}. An example is shown in Figure~\ref{fig:rendersubset} taking one of the rendered examples in \S\ref{sec:basic}, selected half of the domain with the mouse, pressed 'p' to plot only the selected particles. The result is the plot shown.
\begin{figure}[h]
\begin{center}
\includegraphics[angle=270,width=0.5\textwidth]{rendersubset.pdf}
\caption{Example of rendering using only a subset of the particles. Here I have selected only particles on the right hand side of the plot using the mouse.}
...
option from the (p)age options menu (\S\ref{sec:optionspage}) is set. 
...
\subsubsection{ Changing the text of the time legend}
...
Also prompts to allow particles of non-SPH types to be
...
\subsubsection{ Changing graph markers for each particle type}
 The full list of available markers is given in the
PGPLOT user guide. 
...
\subsubsection{ Plotting using lines instead of dots (e.g. for energy vs time plots)}
 When set, this option plots a line connecting the (gas only) particles
...
relative closeness of the particles in memory and in physical space in higher dimensions. The line colours and styles can be changed.
...
Enter number of circles to draw ([0:100], <cr>=0):1
Enter particle number to plot circle around ([1:959], <cr>=1): 868
\end{verbatim}
...
\subsubsection{ Plotting in different co-ordinate systems (e.g. cylindrical co-ordinates)}
\label{sec:geom}
...
\subsubsection{ Changing the exact solution line style \& colour}
 These options allow you to change the colour and line
...
\subsubsection{ Plotting in a comoving reference frame}
 Co-ordinate limits are centred on the
...
\subsubsection{ Restoring plot limits to their minimum and maximum values in the current dump file}
...
\subsubsection{ Using a different colour scheme}

Changes the colour scheme used on rendered
...
With this option set, rendered plots are
...
This option means that the SPH interpolations are
normalised. In general this leads to smoother rendering but also means that edges and surfaces appear
more prominently (and a bit strange). Note that in 3D this option only affects cross-section slices (as it is a bit meaningless to normalise a column-integrated or opacity-rendered plot). 
...
\subsubsection{ Using 3D surface rendering}
 3D surface rendering performs a ray-trace through
...
writing the pixel map directly to a .ppm file (called splash\_\#\#\#\#\#.ppm where the number
...
\subsection{Fix the annoying PGPLOT file naming system?} 
