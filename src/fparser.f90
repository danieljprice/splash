...
FUNCTION evalf (i, Val) RESULT (res)
 ...
 DO IP=1,Comp(i)%ByteCodeSize
    ...
    CASE  (cgamma); if (abs(Comp(i)%Stack(SP) + abs(nint(Comp(i)%Stack(SP)))) < tiny(0._rn)) &
       then; EvalErrType=8; res=zero; return; ENDIF
...
!            write(*,*)'3. F(b:e) = "fcn(...)"'
          two_args = TwoArgs(F,b2+1,e-1,m)
          if (n == cAtan2 .and. two_args) then
